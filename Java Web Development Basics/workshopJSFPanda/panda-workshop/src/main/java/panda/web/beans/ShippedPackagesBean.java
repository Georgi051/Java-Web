...
public class ShippedPackagesBean {
    ...
    private void initPackages() {
        this.packages = this.packageService.findAllPackagesByStatusWithDate(Status.Shipped)
                .stream()
                .map(p ->{
    ...
    public void changeStatus(String id) throws IOException {
        this.packageService.changePackageStatus(id);
        FacesContext.getCurrentInstance().getExternalContext().redirect("/faces/view/shipped.xhtml");
    }
}
